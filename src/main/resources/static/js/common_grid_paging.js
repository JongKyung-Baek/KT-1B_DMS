 ...
 */
function initPage(param, pagerId, pI, pit, gridParamId){
...
//	var totalPage = Math.ceil(totalSize/pageCount);
	...
	if(customPageInfo){
		//////////////////////////////////////////////////////////////////////////////////////////
		var base = parseInt($grid.getGridParam('page'),10)-1 ;
		if(base < 0) { base = 0; }
		base = base*parseInt($grid.getGridParam('rowNum'),10);
		var from = base+1;
		var to = Number(base) + Number($('#'+gridId).getGridParam('rowNum'));
...
//			}
	...
	table+= "<tr>";
	table+= "<td width='29%' style='text-align: left'>";
	if(undefined !== $grid.getGridParam('pager')) {
		var cookieRowNum = $.trim(getCookie("rowNum"));

		if(null != cookieRowNum && undefined !== cookieRowNum && "" !== cookieRowNum) {
			cookieRowNum = parseInt(cookieRowNum, 10);
		}
		else {
			cookieRowNum = 10;
		}

		table+= '<select onchange="changeRowNum(\'' + gridId + '\', ' + gridParamId + ', this.value)" id="' + gridId + 'changePageSelect">';
		$.each($grid.getGridParam('rowList'), function() {
	...
	if(undefined !== $grid.getGridParam('pager')) {
		$("#" + gridId + "changePageSelect").select2({
...
function changeRowNum(gridId, gridParam, v) {
	pagingParam = {
			postData: {
				size : v,
				page : 1
			...
			size : v,
			url : gridParam.url,
			gridId: gridParam.gridId,
			formId : gridParam.formId
	}
	setCookie( "rowNum", v, 1000 );
//	pageCount = v;
	currentPage = 1;
	$('#'+gridId+'changePageSelect').val(v);
	searchList(pagingParam);
}
