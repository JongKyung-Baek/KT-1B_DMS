...
$.extend($.jgrid,{
	filterRefactor : function ( p  )  {
		/*ruleGroup : {}, ssfield:[], spldocselect:",", groupOpSelect:"OR"*/
			...
			if(filters.rules && filters.rules.length) {
				...
				for(k=0; k < rules.length; k++) {
					...
					if($.inArray(rule.filed, p.ssfield)) {
						ssdata = rule.data.split(p.spldocselect);
						if(ssdata.length > 1) {
	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			function getCurrPos() {
				...
				if($t.p.multiselect && rp_ge[$t.p.id].eddocselected) {
					var arr = [];
...
//module begin
	...
	$.fn.fmatter.showlink = function(cellval, opts) {
		...
		if($.fmatter.isString(cellval) || $.fmatter.isNumber(cellval)) {	//add this one even if docs blank string
