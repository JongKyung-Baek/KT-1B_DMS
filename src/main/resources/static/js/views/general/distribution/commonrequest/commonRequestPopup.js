...
$(function() {
	...
	$('#companyCd').on('select2:select', function (e) {
		callAjax(param, "/general/authorization/selectDistributionCompanyUserCombo?companyCd="+e.params.data.id+"&businessAreaCd="+$("#businessAreaCd").val(), function(data){
			$("#companyUserCd").html('');
		...
		//구매담당자
		var param;
		callAjax(param, "/general/authorization/getPurchaseInfo?companyCd="+e.params.data.id + "&businessAreaCd=" + $("#businessAreaCd").val(), function(data){
	...
	$('#companyUserCd').on('select2:select', function (e) {
		$.ajax({
			url: "/general/distribution/commonRequest/getUserEmail"
		  , type : "POST"
		  , cache: false
		  , dataType : "json"
		  , data : {
...
function validate(){
	...
	if(protectYn){
		...
		if(protectUserCnt == 2){
			...
			callAjax(param, '/general/distribution/commonRequest/verificationProtectUser', protectUserCallback);
	//		if(twoDevelopment){
...
//	console.log(jsonArr);
	...
	callAjax(param, '/general/distribution/commonRequest/approvalRequest', requestCallback)
}
