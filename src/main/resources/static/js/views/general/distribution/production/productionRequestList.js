...
function loadProductionRequestExplorerTree() {
	callAjax({}, "/general/distribution/productionRequest/selectTree", function(response) {
...
function requestDistribute(){
	...
	if($("#"+gridId).getGridParam('selarrrow').length < 1){
		...
		openDialogPopup("/general/distribution/commonRequest/productionRequestPopup", {protectYn: protectYn, requestType: requestType, approvalRequestType : "DISTRIBUTION"}, "popupDialog", 'l', 685, true, 'popup-common popup-request');
	}
}
...
function requestPrint(){
	...
	if($("#"+gridId).getGridParam('selarrrow').length < 1){
		...
		openDialogPopup("/general/distribution/commonRequest/commonPrintRequestPopup", {protectYn: protectYn, requestType: requestType, approvalRequestType : "DISTRIBUTION"}, "popupDialog", 'l', popupHeight, true, 'popup-common popup-print-request');
	}
}
...
function viewing(){
	openDialogPopup("/general/common/viewer/openViewerPopup", {}, "popupDialog", 'l', 600);
}

function searchAll(){
	openDialogPopup("/general/distribution/commonRequest/searchAllPopup", {type:'PRODUCTION'}, "searchAllPopup", 's', 600);
}

function formatOpenView(cellValue, options, rowdata, action){
	return '<a onclick="openDialogPopup(\'/general/distribution/productionRequest/productionFilePopup\', { objectId: \'' + rowdata["objectId"] + '\', objectNo: \'' + rowdata["objectNo"] + '\', requestNo: \'' + rowdata["requestNo"] + '\' }, \'popupDialog\', \'l\', 720, true, \'popup-common popup-production-file\')">' + cellValue + '</a>';
}
...
function updateFile(){
	...
	var openPopup = function(param) {
		openDialogPopup(
			"/general/distribution/productionRequest/productionRegisterPopup",
			param,
			"popupDialog",
			'l',
			popupHeight,
	...
	if ($.trim(popupParam.objectId) !== "") {
		callAjax(
			...
			"/general/distribution/productionRequest/selectRevisionUsers",
			function(res){
