...
// 		$('.gnbDepthArea .depth1 #MENU_001').addClass('open tgcp'); // left menu check
		$('.gnbDepthArea #MENU_064').addClass('current'); // left menu check
...
// 		setTabIndex();
	...
	function setTabIndex(){
		var nav = [];
		if(gridParam.gridId === 'gridProductionDisposalList'){
			$('.tabArea ul > li').eq(0).addClass('current');
			$(".titleBox span").text("폐기요청");
			nav = '<span>생산기술자료 사내배포</span><span>폐기요청/현황</span><span>폐기요청</span>';
		}else if(gridParam.gridId === 'gridProductionDisposalStatusList'){
			$('.tabArea ul > li').eq(1).addClass('current');
			$(".titleBox span").text("폐기현황");
			nav = '<span>생산기술자료 사내배포</span><span>폐기요청/현황</span><span>폐기현황</span>';
		}
		$(".navBox").html(nav);
	}
	</script>
