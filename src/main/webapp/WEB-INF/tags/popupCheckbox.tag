...
<c:forEach items="${options}" var="option" >
	<c:choose>
		<c:when test="${checkedValue == option.comboVal }">
			<input type="checkbox" id="${name }" name="${name }" value="${option.comboVal }" checked="checked" ${disabled }>
		</c:when>
		<c:otherwise>
			<input type="checkbox" id="${name }" name="${name }" value="${option.comboVal }" ${disabled }>
		</c:otherwise>
	</c:choose>
	<label for="${name }">${option.comboLabel }</label>
	<script>
