...
<script type="text/javascript" src="${pageContext.request.contextPath}/resources/js/views/general/distribution/disposalacceptance/approvalPopup.js"></script>
<!-- 폐기 승인 -->
<script>
var popupGridParam;
...
function setPopupGridParam(){
	popupGridParam = {
			...
			url : '/general/distribution/disposalacceptance/selectPopupList',
			pagerId : '',
			size : 9999,
			page : 1,
			multiSelect : false,
...
function fileDownload(destroyRequestNo, destroyFileSeq){
	...
	if (frame) {
		frame.src = "/general/distribution/disposalacceptance/fileDownload?destroyRequestNo="
...
function saveApproval(flag){
	if('A' == flag){
		confirmMessage(g_msg("msg.approvalDestory"), function(){
			...
			callAjax(param, "/general/distribution/disposalacceptance/saveApproval", function(response){
