<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
	<%@ taglib prefix="spring" uri="http://www.springframework.org/tags" %>
		...
		<script>
			...
			function openDrawingFileViewer(objectId, fileNo) {
				if (!objectId) {
					alertMessage("파일 정보를 찾을 수 없습니다.");
					return;
				}
				openFile("OBJECT", "DRAWING", popupRequestNo || null, objectId, fileNo || null, "N");
			}

			function isDownloadableDrawingFile(rowdata) {
			...
			function formatDrawingFileName(cellValue, rowdata, useSubFileNo) {
				var name = cellValue || "";
				if (name === "") {
					return "";
				}
				if (!isDownloadableDrawingFile(rowdata || {})) {
			...
			function initDrawingFileGrid(gridId, rows) {
				var $grid = $("#" + gridId);
				var useSubFileNo = gridId === "gridDrawingSubFile";
				...
				$grid.jqGrid({
					...
					colModel: [
						{ name: "fileNo", label: "파일순번", width: 90, align: "center", sortable: false },
						{
							...
							formatter: function (cellValue, options, rowdata) {
								return formatDrawingFileName(cellValue, rowdata || {}, useSubFileNo);
							}
						},
					...
					beforeSelectRow: function (rowId, e) {
						return $(e.target).closest(".drawing-file-link").length === 0;
					}
				});
			...
			function loadApprovalStatus() {
				if (!popupObjectId) {
					...
					initApprovalStatusGrid("gridDrawingApproverStatus", emptyRows);
					initApprovalStatusGrid("gridDrawingReviewerStatus", emptyRows);
					return;
				}
				callAjax(
					{ objectId: popupObjectId },
					"/general/distribution/drawingRequest/approveStatusRows",
					function (response) {
						...
						if (!reviewerRows.length) {
							reviewerRows = [{ approver: "-", status: "-", comment: "-", editable: "N" }];
						}
						initApprovalStatusGrid("gridDrawingApproverStatus", approverRows);
						initApprovalStatusGrid("gridDrawingReviewerStatus", reviewerRows);
					},
					"json"
				);
			}
			...
			function saveApprovalComment(gridId, rowId) {
				...
				callAjax(
					...
					"/general/distribution/drawingRequest/saveApprovalComment",
					function (response) {
			...
			function downloadSelectedDrawingFile(gridId) {
				var $grid = $("#" + gridId);
				...
				for (var i = 0; i < selectedRows.length; i++) {
					...
					if (!isDownloadableDrawingFile(data || {})) {
						blockedFiles.push((data && (data.fileViewNm || data.orgFileNm || data.fileNm || data.fileName)) || "알 수 없는 파일");
						continue;
					}
					var url = "/general/distribution/drawingRequest/downloadFile?objectId=" + encodeURIComponent(data.objectId);
				...
				downloadDrawingFilesSequentially(requests, 0, blockedFiles);
			}

			function downloadDrawingFilesSequentially(requests, idx, blockedFiles) {
				...
				fetch(req.url, { method: "GET", credentials: "same-origin" })
					...
					.finally(function () {
						setTimeout(function () {
							downloadDrawingFilesSequentially(requests, idx + 1, blockedFiles);
						}, 150);
					});
			}
			...
			function isDrawingFileProcessing(status) {
			...
			function getDrawingMainRowsForDisplay(rows) {
				var list = Array.isArray(rows) ? rows : [];
				if (!list.length) {
					return list;
				}
				var status = list[0] && list[0].processingStatus;
				if (!isDrawingFileProcessing(status)) {
					return list;
				}
				return [{
					fileNo: "",
			...
			$(function () {
				mainFileRows = getDrawingMainRowsForDisplay(mainFileRows);
				if (mainFileRows.length && mainFileRows[0].orgFileNm === "파일 등록중입니다.") {
					subFileRows = [];
				}
				loadApprovalStatus();
				initDrawingFileGrid("gridDrawingMainFile", mainFileRows);
				initDrawingFileGrid("gridDrawingSubFile", subFileRows);

				$(document).on("click", ".drawing-file-popup .drawing-file-link", function (e) {
					e.preventDefault();
					...
					openDrawingFileViewer(objectId, fileNo);
				});

				$(document).on("click", ".drawing-file-popup .approval-comment-save", function (e) {
					e.preventDefault();
		...
		<style>
			.drawing-file-popup .popupMeta {
				margin: 6px 0 14px;
				font-size: 14px;
				color: #2f3a55;
			}

			.drawing-file-popup .sectionBlock {
				margin-top: 12px;
			}

			.drawing-file-popup .popupCard {
				background: #fff;
			...
			.drawing-file-popup .dialogToolbar {
				height: 42px;
				padding: 0 14px;
			...
			.drawing-file-popup .dialogToolbar .gridTitle {
				font-size: 14px;
				font-weight: 700;
				color: #2f3a55;
			}

			.drawing-file-popup .dialogToolbar .listCount {
				margin-left: 8px;
				color: #6b778c;
				font-size: 12px;
			}

			.drawing-file-popup .gridContainer {
				padding: 0;
			}

			.drawing-file-popup .gridContainer .ui-jqgrid {
				width: 100% !important;
				height: auto !important;
				border: 0;
			}

			.drawing-file-popup .gridContainer .ui-jqgrid-view,
			.drawing-file-popup .gridContainer .ui-jqgrid-hdiv,
			.drawing-file-popup .gridContainer .ui-jqgrid-bdiv {
				width: 100% !important;
			}
			...
			.drawing-file-popup .mainFileSection .gridContainer .ui-jqgrid-bdiv {
				height: 52px !important;
			}

			.drawing-file-popup .subFileSection .gridContainer .ui-jqgrid-bdiv {
				height: 174px !important;
			}

			.drawing-file-popup .ui-jqgrid .ui-jqgrid-hdiv {
				background: #fafafa;
				border-color: #909090;
			}

			.drawing-file-popup .ui-jqgrid .ui-jqgrid-htable th {
				background: #fafafa;
				color: #333333;
				font-weight: 700;
				border-color: #e0e0e0;
			...
			.drawing-file-popup .ui-jqgrid tr.jqgrow td {
				height: 36px;
				border-color: #edf1f7;
			}

			.drawing-file-popup .ui-jqgrid .ui-jqgrid-bdiv tr.ui-state-hover,
			.drawing-file-popup .ui-jqgrid .ui-jqgrid-bdiv tr:hover,
			.drawing-file-popup .ui-jqgrid .ui-jqgrid-bdiv tr:nth-child(odd).ui-state-hover {
				background: rgba(0, 127, 175, 0.25);
			}

			.drawing-file-popup .ui-jqgrid .ui-jqgrid-htable th.ui-state-hover,
			.drawing-file-popup .ui-jqgrid .ui-jqgrid-htable th:hover,
			.drawing-file-popup .ui-jqgrid .ui-jqgrid-htable .ui-jqgrid-labels th.ui-state-hover,
			.drawing-file-popup .ui-jqgrid .ui-jqgrid-htable .ui-jqgrid-labels th:hover {
				border-color: rgba(0, 127, 175, 0.25);
				background: rgba(0, 127, 175, 0.25);
				color: #333333;
			}
			.drawing-file-popup .ui-jqgrid input.cbox,
			.drawing-file-popup .ui-jqgrid input.cbox:hover,
			.drawing-file-popup .ui-jqgrid input.cbox:focus,
			.drawing-file-popup .ui-jqgrid input.cbox:focus-visible,
			.drawing-file-popup .ui-jqgrid input.cbox:active {
				box-shadow: none !important;
				outline: none !important;
			}
		</style>

		<div class="dialogContent drawing-file-popup popup-base popup-actions-center popup-type-form-grid">
			<div class="popupHero">
				<h2>파일 정보</h2>
			</div>

			<div class="popupMeta">
				문서번호: <strong>${drawingNo}</strong>
			</div>
				...
				<div class="dialogToolbar">
					...
					<div class="right">
						<button type="button" class="ui-button ui-corner-all bottomBtn"
							onclick="saveApprovalCommentByGrid('gridDrawingApproverStatus')">저장</button>
					</div>
				</div>
				<div class="gridContainer">
					<table id="gridDrawingApproverStatus"></table>
				</div>
			</div>
			...
			<div class="section popupCard sectionBlock approvalSection">
				<div class="dialogToolbar">
					...
					<div class="right">
						<button type="button" class="ui-button ui-corner-all bottomBtn"
							onclick="saveApprovalCommentByGrid('gridDrawingReviewerStatus')">저장</button>
					</div>
				</div>
				<div class="gridContainer">
					<table id="gridDrawingReviewerStatus"></table>
				</div>
			</div>
			...
			<div class="section popupCard sectionBlock mainFileSection">
				<div class="dialogToolbar">
					...
					<div class="right">
						<button type="button" class="ui-button ui-corner-all bottomBtn"
							onclick="downloadSelectedDrawingFile('gridDrawingMainFile')">다운로드</button>
					</div>
				</div>
				<div class="gridContainer">
					<table id="gridDrawingMainFile"></table>
				</div>
			</div>
			...
			<div class="section popupCard sectionBlock subFileSection">
				<div class="dialogToolbar">
					...
					<div class="right">
						<button type="button" class="ui-button ui-corner-all bottomBtn"
							onclick="downloadSelectedDrawingFile('gridDrawingSubFile')">다운로드</button>
					</div>
				</div>
				<div class="gridContainer">
					<table id="gridDrawingSubFile"></table>
				</div>
			</div>
		</div>
