...
<script type="text/javascript" src="/resources/js/views/general/distribution/drawingRequest/drawingRegisterPopup.js"></script>
...
<script>
    ...
    function saveX() {
        ...
        var prevDrawingNo = document.getElementById("prevDrawingNo").value;
        ...
        formData.append("prevDrawingNo", prevDrawingNo);
        formData.append("distributionPoint", distributionPoints.join(","));
        formData.append("objectId", hiddenObjectId);
        console.log("hiddenObjectId: ", hiddenObjectId);
        formData.append("isNewRevision", "true");
        formData.append("distributeTypeCd", distributeType);

        callAjaxUpload(  formData, "/general/distribution/drawingRequest/uploadDrawingRegisFile", requestCrXCallback);
    }

    function requestCrXCallback(response){
        if(response.success){
//		if (curSendIndex < emptyArray2.length)
//			callAjax(emptyArray2[curSendIndex++], "/general/unregisted/request/saveUnregisterFileX", requestCrXCallback);
//		else
...
//	console.log("param.filePath : ", param.filePath)
        ...
        //callAjax(param, "/general/unregisted/request/saveUnregisterFileX", saveUnregisterFileXCallback);
...
<div class="dialogContent drawingRevisionUpdatePopup drawingRegisterPopup popup-base popup-actions-center">
    <div class="popupHero">
        <h2>도면 리비전 등록</h2>
    ...
    <form id="formDrawingRegisterPopup">
        ...
        <ul class="section popupCard popupFormGrid">
            ...
            <%-- 도번 --%>
            <li class="half">
                <label>도번</label><input type="text" name="prevDrawingNo" id="prevDrawingNo" value="${drawingNo}" readonly>
            </li>

            <%-- 도번 REV --%>
            <li class="half">
                <label>현재 REV</label><input type="text" name="curRevNo" id="curRevNo" value="${curRevNo}" readonly>
            </li>

            <%-- 도번 REV --%>
            <li class="half">
                <label>신규 REV</label><input type="text" name="newRevNo" id="newRevNo" value="${newRevNo}" readonly>
            </li>

            <%-- 도번 도번 --%>
            <li class="half">
                <custom:popupSelectBox options="${distributeTypeCd}" name="distributeTypeCd" label="grid.fileType" id="distributeTypeCd" />
            </li>

            <%-- 자료형 --%>
            <li class="half">
                <custom:popupInputText name="dataName" id="dataName" label="form.dataName" value="${drawingNm}"/>
            </li>
