...
<head>
	...
	<script type="text/javascript" src="${pageContext.request.contextPath}/resources/js/views/general/distribution/dxf/dxfRequestList.js"></script>
	<script>
		...
		function setGridParam() {
			gridParam = {
				...
				url: '/general/distribution/dxfRequest/selectList',
		...
		function request() {
			...
			openDialogPopup('/general/distribution/dxfRequest/requestPopup', {}, 'popupDialog', 'l', '800');
		}
		...
		function printRequest() {
			...
			openDialogPopup('/general/distribution/dxfRequest/printRequestPopup', {}, 'popupDialog', 's', '330');
		}

		function formatViewFile(cellValue, options, rowdata, action) {
			return '<a onclick="openDialogPopup(\'/general/distribution/dxfRequest/dxfFilePopup\', { objectId: \'' + rowdata["objectId"] + '\', objectNo: \'' + rowdata["objectNo"] + '\', requestNo: \'' + rowdata["requestNo"] + '\' }, \'popupDialog\', \'l\', 720, true, \'popup-common popup-dxf-file\')">' + cellValue + '</a>';
		}
		...
		function upload() {
			...
			openDialogPopup(
				"/general/distribution/dxfRequest/dxfRegisterPopup",
		...
		function deleteDxf() {
			...
			confirmMessage(g_msg('msg.requestDelete'), function () {
				$(this).dialog("close");
				callAjax(param, "/general/distribution/dxfRequest/delete", deleteDxfCallback, 'json');
			});
		}
		...
		function approveDxf() {
			...
			confirmMessage('승인하시겠습니까?', function () {
				$(this).dialog("close");
				callAjax(param, "/general/distribution/dxfRequest/approve", approveDxfCallback, 'json');
			});
		}
		...
		function showDxfStatusDetail(rowId) {
			...
			openDialogPopup(
				"/general/distribution/commonRequest/approvalStatusPopup",
				{
					objectId: rowdata.objectId,
					approveUrl: "/general/distribution/dxfRequest/approveStatusMessage",
					requestType: "DXF"
				},
				"popupDialog",
				"s",
		...
		function openDxfFilePopup(objectId, objectNo, requestNo) {
			openDialogPopup(
				"/general/distribution/dxfRequest/dxfFilePopup",
