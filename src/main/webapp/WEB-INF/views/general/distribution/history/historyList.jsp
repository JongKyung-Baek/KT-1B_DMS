...
var gridId = 'gridDistributionHistoryList';
	function setGridParam(){
		gridParam = {
				...
				url : '/general/distribution/history/selectList',
	...
	function stopDeal(){
		openDialogPopup("/general/distribution/history/stopDealPopup", {}, "popupDialog", 'm', 650, true, 'popup-common');
	}
	...
	function destroy(){
		...
		if(isSuccess){
			openDialogPopup("/general/distribution/history/destroyPopup", {destroyType: destroyType}, "popupDialog", 'm', 700, true, 'popup-common');
		}
	}
...
var strUserCd = '<spring:escapeBody htmlEscape="false" javaScriptEscape="true">${sessionUser.userCd}</spring:escapeBody>';
		...
		if(isSuccess){
			confirmMessage(g_msg("msg.confirmUnderDestroy"), function(){ //폐기중으로 변경 하시겠습니까?
				...
				$.each($("#"+ gridId).getGridParam('selarrrow'), function(index, item){
					...
					//console.log(param);
					callAjax(param, '/general/distribution/history/underDestroy', underDestroyCallback);
				});
			});
		}
	}
	...
	function searchAll(){
		openDialogPopup("/general/distribution/commonRequest/searchAllPopup", {type:'HISTORY'}, "searchAllPopup", 'm', 600);
	}
