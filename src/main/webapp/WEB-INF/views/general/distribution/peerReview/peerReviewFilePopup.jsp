...
<script>
	...
	function loadApprovalStatus() {
		...
		callAjax(
			...
			"/general/distribution/peerReview/approveStatusRows",
			function (response) {
	...
	function saveApprovalComment(gridId, rowId) {
		...
		callAjax(
			...
			"/general/distribution/peerReview/saveApprovalComment",
			function (response) {
	...
	function downloadSelectedPeerReviewFile(gridId) {
		...
		$.each(selectedRows, function (idx, rowId) {
			...
			var url = "/general/distribution/peerReview/downloadFile?objectId=" + encodeURIComponent(objectId);
			if (fileNo) {
