	...
	<script>
		...
		function setGridParam() {
			gridParam = {
				...
				url: '/general/distribution/swRequest/selectList',
		...
		function formatViewFile(cellValue, options, rowdata, action) {
			return '<a onclick="openDialogPopup(\'/general/distribution/swRequest/swFilePopup\', { objectId: \'' + rowdata["objectId"] + '\', swNo: \'' + rowdata["swNo"] + '\', requestNo: \'' + rowdata["requestNo"] + '\' }, \'popupDialog\', \'l\', 720, true, \'popup-common popup-sw-file\')">' + cellValue + '</a>';
		}
			...
			if (cellValue == "Y") {
				...
				return cellValue;
					...
					function openDialog(objectId) {
						openDialogPopup("/general/distribution/commonRequest/protectPopup", { objectId: objectId, objectType: "SW" }, "popupDialog", 'm', 360, true, 'popup-common popup-protect');
					}

					// 2023.07.24 기범추가 ( 등록 버튼 생성 )
					function upload() {
						var treeCd = $("#formSwRequest [name='swTreeCd']").val() || $("#formSwRequest [name='swTreeCd_treeHidden']").val() || "";
						var url = "/general/distribution/swRequest/regist";
		...
		function deleteSW() {
			...
			confirmMessage(swRequestMessage("feature.techList.withdraw.confirm", "철회하시겠습니까?"), function () {
				$(this).dialog("close");
				callAjax(param, "/general/distribution/swRequest/delete", deleteSWCallback, 'json');
			});
		}
		...
		function approveSW() {
			...
			confirmMessage(swRequestMessage("feature.techList.approval.confirm", "승인하시겠습니까?"), function () {
				$(this).dialog("close");
				callAjax(param, "/general/distribution/swRequest/approve", approveSWCallback, 'json');
			});
		}
		...
		function showSWStatusDetail(rowId) {
			...
			openDialogPopup(
				"/general/distribution/commonRequest/approvalStatusPopup",
				{
					...
					approveUrl: "/general/distribution/swRequest/approveStatusMessage",
					requestType: "SW"
				},
				"popupDialog",
				"s",
