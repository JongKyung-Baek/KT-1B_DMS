...
<script type="text/javascript" src="/resources/js/views/inside/unregisted/request/registerPopup.js"></script>
...
function isValidationX(){
	...
	var fileInput = document.getElementById("unregFile");
...
function saveX() {
	...
	// param.append('file', emptyArray2[0]);
	// param.append('formUnRegisterPopup', JSON.stringify($('#formUnRegisterPopup').serializeObject()));
	// console.log(param);
	//
	// var fileInput = document.getElementById("unregFile");
	// var file = fileInput.files[0];
	...
	// formData.append("file", file);
	 ...
	 //callAjax( {paramList: emptyArray2}, "/inside/unregisted/request/saveUnregisterFileX", requestCrXCallback);
	...
	formData.append("objectType", "UNREG");
	formData.append("fileNm", "asdasd");

	callAjaxUpload(  formData, "/inside/unregisted/request/uploadUnregFile", requestCrXCallback);
}
...
<%--			<label><spring:message code="form.fileUpload"/></label>--%>
			<div>
				...
				<input type="text" name="fileName" id="fileName"/><button class="ui-button ui-corner-all fileUploadBtn" onclick="fileUpload()"><spring:message code="btn.fileUpload"/></button>
			</div>
		</li>
...
<%--&lt;%&ndash;            <OBJECT ID="uploadActiveX" CODEBASE="https://localhost:8080/resources/install/ExtUpDown.cab#version=1,2015,527,1" CLASSID="CLSID:2D77F2E4-1C6F-475C-B83E-29EFEF9D1659" width="680" height="320" style="">&ndash;%&gt;--%>
<%--                &lt;%&ndash; <PARAM NAME="uploadServerIp"       VALUE="${updownCabUrl }"> &ndash;%&gt;--%>
